...
UI.ServerTexture = function ( mapping ) {
	...
	canvas.addEventListener( 'click', async ( event ) => {
		...
		if(!dom.disabled) {
			//input.click();
			let filePickerDiv = new UI.Div();
			filePickerDiv.setWidth("300px");
			let imagefiles = await getCurrentModel().getImageFiles();
			imagefiles.forEach((image) => {
				let imageRow = new UI.Row();
				imageRow.add(new UI.Button( image ).setWidth("100%").onClick(() => {
					loadFile(image, (texture) => {
						this.texture = texture;
						this.onChangeCallback();
					});
				}));
				filePickerDiv.add(imageRow);
			});
			modal.dom.children[0].style.width = filePickerDiv.dom.style.width;
			modal.show(filePickerDiv);
		}
...
UI.ServerTexture.prototype = Object.create( UI.Element.prototype );
UI.ServerTexture.prototype.constructor = UI.Texture;

UI.ServerTexture.prototype.getValue = function () {

	return this.texture;

};

UI.ServerTexture.prototype.setValue = function ( texture ) {
...
UI.ServerTexture.prototype.onChange = function ( callback ) {
...
UI.ServerTexture.prototype.setEnabled = function( enabled ) {
...
UI.ServerTexture.prototype.getEnabled = function( ) {
